...
def test_start_game_(Game_Controller_with_start):
    ...
    assert len(Game_Controller_with_start.pool) == 100
...
def test_remove_word(Game_Controller_with_start):
    ...
    for i in range(99):
        assert Game_Controller_with_start.status == 'waiting_round'
        now_len = len(Game_Controller_with_start.pool)
        Game_Controller_with_start.remove_word('correct', 1.0)
        assert now_len == len(Game_Controller_with_start.pool) + 1
    assert Game_Controller_with_start.status == "show_stats"
